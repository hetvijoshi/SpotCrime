...
const ReportCrime = ({ onSubmit, onClose }) => {
    ...
    useEffect(() => {
        const fetchCrimeCategories = async () => {
            try {
                const response = await axios.get('http://localhost:3001/api/crimecategory');
    ...
    const getAddressSuggestions = async (input) => {
        try {
            if (input.length > 2) {
                const response = await axios.get(`http://localhost:3001/api/autocomplete?input=${encodeURIComponent(input)}`);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.post('http://localhost:3001/api/crimes', newCrime);
            onSubmit(newCrime)
            // Reset form fields
            setAddress(null)
            setDateTime(dayjs())
